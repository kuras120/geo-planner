...
1. [pending] Replace mixed root tasks with `setup`, `setup-ci`, component
   `verify-*` tasks, aggregate `verify`, `assemble`, and `ci`.
2. [pending] Keep local installation mutable with `npm install`; make CI
   installation frozen with `npm ci` and install the required Playwright browser
   dependencies.
3. [pending] Change backend verification from `:backend:test` to
   `:backend:check` and confirm ktlint remains attached exactly once.
4. [pending] Update `.github/workflows/build.yml` to run `mise run ci` after
   checkout and mise tool setup.
5. [pending] Update durable repository setup documentation and README command
   examples to the accepted task names.
6. [pending] Run the individual tasks and the aggregate CI task, recording any
