...
6. Do not change a plan from `PROPOSED` or implement it unless the owner explicitly approves implementation or explicitly requests planning and execution together for a named scope. That combined request authorizes ordinary reversible decisions inside the named scope, but a newly discovered material owner decision still blocks dependent work until accepted. Editing, reviewing, or expanding a plan is not approval.
